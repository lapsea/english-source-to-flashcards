...
- Size the card from the remaining flex space between the header and controls, not from `100vh` or `100dvh`; embedded mobile previews can report a viewport taller than their visible canvas. Give the track and each slide an explicit height so a `height: 100%` card is fully resolved on its first render, before the user flips it.
